            ...
            if message.isPrivate && conversationUIModel.isSentByCurrentUser(message),
               let status = deliveryStatus {
                if case .failed = status {
                    Text(verbatim: "\(status.bitchatDescription) — \(String(localized: "content.delivery.resend_hint", comment: "Caption suffix pointing at the long-press resend action for failed messages"))")
                        .bitchatFont(size: 11)
